...
> 当前 = v0.10.0，**能跑、1154 测试绿、type-check 干净**（2026-09-17 实测：`npm test` → 91 文件 / 1154 用例全绿）；系统广度已铺得很开（见下方第 4 节"当前状态速览"已更新），完整愿景在设计稿里。
...
npm test               # 跑测试（vitest，1154 条必须全绿）
...
1. **测试不破 + type-check 干净**：改完跑 `npm run verify`（= type-check && 1154 测试）全绿；新功能补测试。已装 git pre-commit 钩子，红了拦截提交（勿用 `--no-verify` 绕过）。
...
- 🟢 **已跑通**：资源/建筑(35)/国策(23)/朝令(12)/事件(沙盒40+故事23)/邦交(NPC动态成长)/存档(游戏内三槽 UI)，**1154 测试 + type-check 干净**（2026-09-17 实测：91 文件 / 1154 用例全绿）。
